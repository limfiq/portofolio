...
const Page = async (props) => {
    ...
    const { data: relatedItems } = await supabase
        .from("community_services")
        .select("title, slug, cover_image, year, location")
        .neq("id", data.id)
    ...
    const contentText = (data.description || "").replace(/<[^>]*>/g, '');
    ...
    return (
        <div className="max-w-4xl mx-auto pt-32 px-4 pb-12 overflow-hidden break-words">
            <div className="mb-12">
                <a href="/pengabdian" className="text-blue-600 hover:underline flex items-center gap-2 mb-8 group w-fit">
                        ...
                        {relatedItems.map((item) => (
                            ...
                            >
                                <div className="relative aspect-video rounded-xl overflow-hidden mb-3 shadow-sm group-hover:shadow-md transition-all">
                                    ...
                                    />
                                    <div className="absolute bottom-2 left-2 px-2 py-0.5 bg-black/60 backdrop-blur-sm rounded text-[10px] text-white">
                                        {item.location}
                                    </div>
                                </div>
